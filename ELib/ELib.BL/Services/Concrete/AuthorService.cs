        ...
        private Func<IQueryable<Author>, IOrderedQueryable<Author>> buildOrderExpression(string orderby, string orderDirection)
        {
           bool isOrderASC = orderDirection == "ASC";
            ...
            if (isOrderASC)
            {
                ...
                {
                    case "FirstName":
                        return q => q.OrderBy(b => b.FirstName);
                    case "LastName":
                        return q => q.OrderBy(b => b.LastName);
                    case "DateOfBirth":
                        return q => q.OrderBy(b => b.DateOfBirth);
            ...
            else
            {
                ...
                {
                    case "FirstName":
                        return q => q.OrderByDescending(b => b.FirstName);
                    case "LastName":
                        return q => q.OrderByDescending(b => b.LastName);
                    case "DateOfBirth":
                        return q => q.OrderByDescending(b => b.DateOfBirth);
